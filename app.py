...
# --- إعدادات الاتصال السحابي (Supabase & Cloudinary) ---
# يتم سحب البيانات من متغيرات البيئة (Environment Variables) في Render أو إعدادها هنا
DATABASE_URL = os.getenv("DATABASE_URL", "")  # رابط اتصال Supabase
...
def get_level(percentage: float) -> str:
    if percentage < 60: return "المستوى 1 (مبتدئ)"
    elif percentage < 75: return "المستوى 2 (متوسط)"
    elif percentage < 85: return "المستوى 3 (متقدم)"
    else: return "المستوى 4 (خبير)"
...
async def admin_login(sap_id: str = Form(...), email: str = Form(...), password: str = Form(...)):
    conn, is_pg = get_db()
    c = conn.cursor()
    query = "SELECT sap_id, name, email FROM admin_settings WHERE id = 1 AND sap_id = %s AND LOWER(email) = %s AND password = %s" if is_pg else \
            "SELECT sap_id, name, email FROM admin_settings WHERE id = 1 AND sap_id = ? AND LOWER(email) = ? AND password = ?"
...
async def update_admin_profile(sap_id: str = Form(...), name: str = Form(...), email: str = Form(...), password: Optional[str] = Form(None)):
    ...
    if password and password.strip():
        q = "UPDATE admin_settings SET sap_id = %s, name = %s, email = %s, password = %s WHERE id = 1" if is_pg else \
            "UPDATE admin_settings SET sap_id = ?, name = ?, email = ?, password = ? WHERE id = 1"
        c.execute(q, (sap_id.strip(), name.strip(), email.strip().lower(), password.strip()))
    else:
        q = "UPDATE admin_settings SET sap_id = %s, name = %s, email = %s WHERE id = 1" if is_pg else \
            "UPDATE admin_settings SET sap_id = ?, name = ?, email = ? WHERE id = 1"
...
async def user_login(sap_id: str = Form(...), password: str = Form(...)):
    conn, is_pg = get_db()
    c = conn.cursor()
    q = "SELECT sap_id, name, role, department FROM users WHERE sap_id = %s AND password = %s" if is_pg else \
        "SELECT sap_id, name, role, department FROM users WHERE sap_id = ? AND password = ?"
...
async def request_password_reset(sap_id: str = Form(...)):
    conn, is_pg = get_db()
    c = conn.cursor()
    q_sel = "SELECT name FROM users WHERE sap_id = %s" if is_pg else "SELECT name FROM users WHERE sap_id = ?"
    ...
    q_ins = "INSERT INTO reset_requests (sap_id, user_name, status) VALUES (%s, %s, 'pending')" if is_pg else \
            "INSERT INTO reset_requests (sap_id, user_name, status) VALUES (?, ?, 'pending')"
...
async def change_password(sap_id: str = Form(...), old_password: str = Form(...), new_password: str = Form(...)):
    conn, is_pg = get_db()
    c = conn.cursor()
    q_sel = "SELECT password FROM users WHERE sap_id = %s" if is_pg else "SELECT password FROM users WHERE sap_id = ?"
    c.execute(q_sel, (sap_id.strip(),))
    row = c.fetchone()
    ...
    q_upd = "UPDATE users SET password = %s WHERE sap_id = %s" if is_pg else "UPDATE users SET password = ? WHERE sap_id = ?"
...
async def upload_multiple_images(files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        if CLOUDINARY_CLOUD_NAME and CLOUDINARY_API_KEY:
            # رفع مباشر إلى Cloudinary واستلام رابط سحابي آمن دائم
            upload_res = cloudinary.uploader.upload(contents, folder="exam_system")
            file_url = upload_res.get("secure_url")
        else:
            os.makedirs("static/uploads", exist_ok=True)
            ext = file.filename.split(".")[-1]
            unique_name = f"{uuid.uuid4().hex}.{ext}"
            with open(f"static/uploads/{unique_name}", "wb") as f:
                f.write(contents)
            file_url = f"/static/uploads/{unique_name}"

        q_ins = "INSERT INTO media_gallery (filename, file_url) VALUES (%s, %s)" if is_pg else \
                "INSERT INTO media_gallery (filename, file_url) VALUES (?, ?)"
...
async def get_gallery():
    ...
    rows = c.fetchall()
    conn.close()
    return [{"id": r[0], "filename": r[1], "url": r[2], "date": str(r[3])} for r in rows]
...
async def add_single_user(sap_id: str = Form(...), name: str = Form(...), password: str = Form(...), role: str = Form("فني"), department: str = Form("عام")):
    ...
    if is_pg:
        ...
        q = """INSERT INTO users (sap_id, name, password, role, department) VALUES (?, ?, ?, ?, ?)
               ON CONFLICT(sap_id) DO UPDATE SET name=excluded.name, password=excluded.password, role=excluded.role, department=excluded.department"""
    c.execute(q, (sap_id.strip(), name.strip(), password.strip(), role.strip(), department.strip()))
    conn.commit()
    conn.close()
...
async def delete_user(sap_id: str):
    conn, is_pg = get_db()
    c = conn.cursor()
    q = "DELETE FROM users WHERE sap_id = %s" if is_pg else "DELETE FROM users WHERE sap_id = ?"
...
async def upload_users_excel(file: UploadFile = File(...)):
    try:
        ...
        for _, row in df.iterrows():
            ...
            role = str(row["الدور"]).strip() if pd.notna(row.get("الدور")) else "مشغل / فني"
            dept = str(row["الإدارة"]).strip() if pd.notna(row.get("الإدارة")) else "عام"
            if sap_id and name:
                if is_pg:
                    q = """INSERT INTO users (sap_id, name, password, role, department) VALUES (%s, %s, %s, %s, %s)
                           ON CONFLICT(sap_id) DO UPDATE SET name=EXCLUDED.name, password=EXCLUDED.password, role=EXCLUDED.role, department=EXCLUDED.department"""
        ...
        conn.commit()
        conn.close()
        return {"status": "success", "message": f"تم استيراد {count} مستخدم بنجاح."}
...
async def get_reset_requests():
    ...
    rows = c.fetchall()
    conn.close()
    return [{"request_id": r[0], "sap_id": r[1], "name": r[2], "date": str(r[3]), "current_password": r[4], "department": r[5]} for r in rows]
...
async def reset_password_action(request_id: int = Form(...), sap_id: str = Form(...), new_password: Optional[str] = Form(None)):
    ...
    if new_password and new_password.strip():
        q_upd = "UPDATE users SET password = %s WHERE sap_id = %s" if is_pg else "UPDATE users SET password = ? WHERE sap_id = ?"
        c.execute(q_upd, (new_password.strip(), sap_id.strip()))
    q_req = "UPDATE reset_requests SET status = 'resolved' WHERE id = %s" if is_pg else "UPDATE reset_requests SET status = 'resolved' WHERE id = ?"
    c.execute(q_req, (request_id,))
    conn.commit()
    conn.close()
    return {"status": "success", "message": "تم تحديث كلمة المرور وإنهاء الطلب."}
...
async def update_exam_validity(exam_id: int, is_active: int = Form(...), valid_until: Optional[str] = Form(None)):
    conn, is_pg = get_db()
    c = conn.cursor()
    q = "UPDATE exams SET is_active = %s, valid_until = %s WHERE id = %s" if is_pg else "UPDATE exams SET is_active = ?, valid_until = ? WHERE id = ?"
...
async def allow_retake(exam_id: int, sap_id: str = Form(...)):
    ...
    c = conn.cursor()
    if is_pg:
        q = "INSERT INTO retake_permissions (exam_id, sap_id, allowed) VALUES (%s, %s, 1) ON CONFLICT(exam_id, sap_id) DO UPDATE SET allowed = 1"
    else:
        q = "INSERT INTO retake_permissions (exam_id, sap_id, allowed) VALUES (?, ?, 1) ON CONFLICT(exam_id, sap_id) DO UPDATE SET allowed = 1"
...
async def upload_excel(exam_name: str = Form(...), duration: int = Form(30), valid_until: Optional[str] = Form(None), departments: str = Form("[]"), file: UploadFile = File(...)):
    try:
        ...
        if not dept_list: dept_list = ["الكل"]
        ...
        if is_pg:
            ...
            c.execute(q_exam, (exam_name, duration, json.dumps(dept_list, ensure_ascii=False), valid_until if valid_until else None))
        ...
        del_q = "DELETE FROM questions WHERE exam_id = %s" if is_pg else "DELETE FROM questions WHERE exam_id = ?"
        ...
        for _, row in df.iterrows():
            ...
            options = [str(row["الاختيار 1"]).strip(), str(row["الاختيار 2"]).strip(), str(row["الاختيار 3"]).strip(), str(row["الاختيار 4"]).strip()]
            correct_option = str(row["الإجابة الصحيحة"]).strip()

            ins_q = "INSERT INTO questions (exam_id, branch, question, image_url, options, correct_option) VALUES (%s, %s, %s, %s, %s, %s)" if is_pg else \
                    "INSERT INTO questions (exam_id, branch, question, image_url, options, correct_option) VALUES (?, ?, ?, ?, ?, ?)"
            c.execute(ins_q, (exam_id, branch, question, image_url, json.dumps(options, ensure_ascii=False), correct_option))
...
async def list_exams(department: Optional[str] = None, sap_id: Optional[str] = None):
    ...
    for r in rows:
        ...
        if sap_id:
            q_cnt = "SELECT COUNT(id) FROM submissions WHERE exam_id = %s AND sap_id = %s" if is_pg else "SELECT COUNT(id) FROM submissions WHERE exam_id = ? AND sap_id = ?"
            c.execute(q_cnt, (exam_id, sap_id.strip()))
            if c.fetchone()[0] > 0: already_submitted = True

            q_perm = "SELECT allowed FROM retake_permissions WHERE exam_id = %s AND sap_id = %s" if is_pg else "SELECT allowed FROM retake_permissions WHERE exam_id = ? AND sap_id = ?"
            c.execute(q_perm, (exam_id, sap_id.strip()))
            perm = c.fetchone()
            if perm and perm[0] == 1: retake_allowed = True

        if department is None or "الكل" in depts or department in depts or department == "عام":
            exams.append({
                "id": exam_id, "name": name, "duration": duration, "departments": depts,
                "is_active": bool(is_active), "valid_until": valid_until, "is_expired": is_expired,
                "already_submitted": already_submitted, "retake_allowed": retake_allowed,
...
async def preview_exam(exam_id: int):
    conn, is_pg = get_db()
    c = conn.cursor()
    q_exam = "SELECT name, duration_minutes, departments, is_active, valid_until FROM exams WHERE id = %s" if is_pg else \
             "SELECT name, duration_minutes, departments, is_active, valid_until FROM exams WHERE id = ?"
    c.execute(q_exam, (exam_id,))
    exam = c.fetchone()
    
    q_qs = "SELECT id, branch, question, image_url, options, correct_option FROM questions WHERE exam_id = %s" if is_pg else \
           "SELECT id, branch, question, image_url, options, correct_option FROM questions WHERE exam_id = ?"
    c.execute(q_qs, (exam_id,))
    questions = [{"id": r[0], "branch": r[1], "question": r[2], "image_url": r[3], "options": json.loads(r[4]), "correct_option": r[5]} for r in c.fetchall()]
    conn.close()
    return {
...
async def update_question(q_id: int = Form(...), branch: str = Form(...), question: str = Form(...),
    ...
    q = "UPDATE questions SET branch = %s, question = %s, image_url = %s, options = %s, correct_option = %s WHERE id = %s" if is_pg else \
        "UPDATE questions SET branch = ?, question = ?, image_url = ?, options = ?, correct_option = ? WHERE id = ?"
    c.execute(q, (branch.strip(), question.strip(), image_url.strip() if image_url else "", json.dumps(options, ensure_ascii=False), correct_option.strip(), q_id))
    conn.commit()
    conn.close()
...
async def get_exam_questions(exam_id: int, sap_id: Optional[str] = None):
    conn, is_pg = get_db()
    c = conn.cursor()
    q_exam = "SELECT duration_minutes, is_active, valid_until FROM exams WHERE id = %s" if is_pg else "SELECT duration_minutes, is_active, valid_until FROM exams WHERE id = ?"
    c.execute(q_exam, (exam_id,))
    exam = c.fetchone()
    if not exam or not exam[1]:
        conn.close()
        raise HTTPException(status_code=403, detail="هذا الامتحان معطل حالياً.")
    
    now = datetime.now().strftime("%Y-%m-%dT%H:%M")
    if exam[2] and exam[2] < now:
        conn.close()
        raise HTTPException(status_code=403, detail="انتهت صلاحية هذا الامتحان.")
    ...
    if sap_id:
        ...
        if c.fetchone()[0] > 0:
            ...
            if not perm or perm[0] != 1:
                conn.close()
                raise HTTPException(status_code=403, detail="لقد قمت بخوض هذا الامتحان سابقاً.")

    q_qs = "SELECT id, branch, question, image_url, options FROM questions WHERE exam_id = %s" if is_pg else "SELECT id, branch, question, image_url, options FROM questions WHERE exam_id = ?"
    c.execute(q_qs, (exam_id,))
    questions = [{"id": r[0], "branch": r[1], "question": r[2], "image_url": r[3], "options": json.loads(r[4])} for r in c.fetchall()]
    conn.close()
...
async def submit_exam(exam_id: int, payload: Dict[str, object]):
    ...
    conn, is_pg = get_db()
    c = conn.cursor()
    q_qs = "SELECT id, branch, correct_option FROM questions WHERE exam_id = %s" if is_pg else "SELECT id, branch, correct_option FROM questions WHERE exam_id = ?"
    c.execute(q_qs, (exam_id,))
    ...
    for q_id, branch, correct_opt in questions:
        if branch not in branch_stats: branch_stats[branch] = {"total": 0, "correct": 0}
    ...
    for branch, stats in branch_stats.items():
        ...
        branch_results[branch] = {"score": stats["correct"], "total": stats["total"], "percentage": round(b_pct, 1), "level": get_level(b_pct)}

    q_sub = """INSERT INTO submissions (exam_id, sap_id, user_name, total_score, total_questions, total_pct, overall_level, branch_details) 
            ...
            """INSERT INTO submissions (exam_id, sap_id, user_name, total_score, total_questions, total_pct, overall_level, branch_details) 
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)"""
    c.execute(q_sub, (exam_id, sap_id, user_name, total_correct, total_q, overall_pct, overall_lvl, json.dumps(branch_results, ensure_ascii=False)))
    
    q_ret = "UPDATE retake_permissions SET allowed = 0 WHERE exam_id = %s AND sap_id = %s" if is_pg else "UPDATE retake_permissions SET allowed = 0 WHERE exam_id = ? AND sap_id = ?"
    ...
    return {
        "user_name": user_name, "total_score": total_correct, "total_questions": total_q,
        "total_pct": round(overall_pct, 1), "overall_level": overall_lvl, "branch_results": branch_results
    }
...
async def get_user_history(sap_id: str):
    conn, is_pg = get_db()
    c = conn.cursor()
    q = """SELECT s.id, e.name, s.total_score, s.total_questions, s.total_pct, s.overall_level, s.branch_details, s.submitted_at 
           FROM submissions s LEFT JOIN exams e ON s.exam_id = e.id WHERE s.sap_id = %s ORDER BY s.submitted_at DESC""" if is_pg else \
    ...
    rows = c.fetchall()
    conn.close()
    return [{"submission_id": r[0], "exam_name": r[1], "score": f"{r[2]}/{r[3]}", "percentage": f"{r[4]:.1f}%", "overall_level": r[5], "branches": json.loads(r[6]), "date": str(r[7])} for r in rows]
...
async def get_exam_leaderboard(exam_id: int):
    conn, is_pg = get_db()
    c = conn.cursor()
    q = """SELECT s.sap_id, s.user_name, u.role, u.department, s.total_score, s.total_questions, s.total_pct, s.overall_level, s.submitted_at 
           FROM submissions s LEFT JOIN users u ON s.sap_id = u.sap_id WHERE s.exam_id = %s ORDER BY s.total_pct DESC, s.submitted_at ASC""" if is_pg else \
    ...
    rows = c.fetchall()
    conn.close()
    return [{"rank": i+1, "sap_id": r[0], "name": r[1], "role": r[2] or "-", "department": r[3] or "-", "score": f"{r[4]}/{r[5]}", "pct": f"{r[6]:.1f}%", "level": r[7], "date": str(r[8])} for i, r in enumerate(rows)]
...
async def get_general_leaderboard():
    ...
    rows = c.fetchall()
    conn.close()
    return [{"rank": i+1, "sap_id": r[0], "name": r[1], "role": r[2] or "-", "department": r[3] or "-", "avg_pct": f"{r[4]:.1f}%", "overall_level": get_level(r[4]), "exams_count": r[5]} for i, r in enumerate(rows)]

@app.get("/api/admin/export-results/{exam_id}")
async def export_results(exam_id: int):
    conn, is_pg = get_db()
    c = conn.cursor()
    q = """SELECT s.sap_id, s.user_name, u.role, u.department, s.total_score, s.total_questions, s.total_pct, s.overall_level, s.branch_details, s.submitted_at 
           FROM submissions s LEFT JOIN users u ON s.sap_id = u.sap_id WHERE s.exam_id = %s ORDER BY s.total_pct DESC""" if is_pg else \
    ...
    for r in rows:
        record = {
            "SAP ID": r[0], "اسم المشترك": r[1], "الدور الوظيفي": r[2] or "-",
            "الإدارة": r[3] or "-", "الدرجة": f"{r[4]}/{r[5]}", "النسبة المئوية": f"{r[6]:.1f}%",
            "المستوى العام": r[7], "التاريخ": str(r[9])
        }
